...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    private static final RedisScript<List> SLIDING_WINDOW_SCRIPT = new DefaultRedisScript<>("""
            ...
            -- bỏ các request đã rơi ra ngoài cửa sổ
            redis.call('ZREMRANGEBYSCORE', KEYS[1], 0, now - window)
            local count = redis.call('ZCARD', KEYS[1])

            if count >= limit then
                -- còn phải chờ tới khi request cũ nhất rời khỏi cửa sổ
    ...
    protected void doFilterInternal(
        ...
        if (!properties.isEnabled() || !request.getRequestURI().endsWith("/auth/login")) {
            filterChain.doFilter(request, response);
            return;
        }

        int limit = properties.getLogin();
        long windowMs = properties.getWindow().toMillis();
        String key = RATE_PREFIX + resolveIdentifier(request) + ":login";
