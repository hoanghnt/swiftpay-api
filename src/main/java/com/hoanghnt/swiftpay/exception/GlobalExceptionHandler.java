...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler({
            com.hoanghnt.swiftpay.client.DownstreamUnavailableException.class,
            io.github.resilience4j.circuitbreaker.CallNotPermittedException.class
    })
    public ResponseEntity<BaseResponse<Void>> handleDownstreamUnavailable(Exception ex) {
        log.warn("Downstream không khả dụng: {}", ex.getMessage());
        return ResponseEntity
                .status(ErrorCode.DOWNSTREAM_UNAVAILABLE.getHttpStatus())
                .body(BaseResponse.error(
                        ErrorCode.DOWNSTREAM_UNAVAILABLE.getCode(),
                        ex.getMessage() != null ? ex.getMessage()
                                : ErrorCode.DOWNSTREAM_UNAVAILABLE.getDefaultMessage()));
    }
